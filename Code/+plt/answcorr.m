function [impCorr, estCorr] = answcorr(ans, dgs, sort_cat, lab_sel)
...
for iL = 1: numel(uniqueRoles)
    ...
    [~, impOrder] = sort(sum(datacorr, 2), 'descend');

    data = isEstablished(idx, :);
    datacorr = corr(data', 'Type', 'Pearson', 'rows', 'complete');
    datacorr(isnan(datacorr(:)))=1;
    [~, impEst] = sort(sum(datacorr, 2), 'descend');

    impOrder = impOrder + boundary(iL);
    impbycorridx = cat(1, impbycorridx, impOrder);

    impEst = impEst + boundary(iL);
    estbycorridx = cat(1, estbycorridx, impEst);

end

impCorr = corr(importance', 'Type', 'Pearson', 'rows', 'complete');
impCorr(isnan(impCorr(:)))=0;
impCorr= impCorr(impbycorridx,impbycorridx);

estCorr = corr(isEstablished', 'Type', 'Pearson', 'rows', 'complete');
estCorr(isnan(estCorr(:)))=0;
estCorr= estCorr(estbycorridx,estbycorridx);
