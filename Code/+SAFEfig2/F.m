...
for k = 1:length(uniqueRoles)
    ...
    if k==1
        A3{k} = axes('position',[V(1) V(2)-V(4)-0.13 V(3)*0.77 V(4)]);
        V = get(A3{k},'Position');
    else
        A3{k} = axes('position',[V2(1)+V2(3)+ 0.07 V2(2) V2(3) V2(4)]);
    end
...
axes('position',[V(1) V(2)-V(4)-0.07 1.2*V(3) V(4)]);
...
axes('position',[V5(1)+V5(3) + 0.07 V5(2) V5(3) V5(4)]);
avg_imp = mean(answ.importance,2);
Ua = unique(dgs.YearsInRole);
Ua = [Ua(end);Ua(1:end-1)];
for k=1:length(RoleList)
    if strcmp(RoleList{k},'PI')
        for j=1:length(Ua)
            dummy = strcmp(RoleList{k},dgs.CurrentRole) & strcmp(dgs.YearsInRole,Ua{j});
            Violin({avg_imp(dummy)}, j , 'ViolinColor',{colour.PI./256},'ViolinAlpha',{Violin_Alpha },'MarkerSize',Violin_Mksize);
        end
    end
end
...
ylim([1 5])
title('Importance score for PIs','FontSize',FontSz)
XL = xlim;
plot(XL,[3 3],'--','Color',[0.8 0.8 0.8])
...
axes('position',[V5(1)+V5(3) + 0.07 V5(2) 1.8*V5(3) V5(4)]);
Nall = [];
for k = 1:length(uniqueRoles)
    idx = strcmp(dgs{:, which_cat}, uniqueRoles{k}); % keep observations for each category
...
addLetter(gca,'k')
%%
saveas(gcf, fullfile(OutF,'Figure4.pdf'));
print(gcf, fullfile(OutF,'Figure4.png'),'-dpng','-r600');
